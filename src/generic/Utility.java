...
public class Utility{
	...
	public static void writeResult(String path,String sheet,int pass,int fail)
	{
		try {
			Workbook v = WorkbookFactory.create(new FileInputStream(path));
			v.getSheet(sheet).getRow(1).getCell(0).setCellValue(pass);
			v.getSheet(sheet).getRow(1).getCell(1).setCellValue(fail);
			v.write(new FileOutputStream(path));
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	public static String getXLData(String path,String sheet,int r,int c)
	{
		...
		try {
			Workbook v=WorkbookFactory.create(new FileInputStream(path));
			s=v.getSheet(sheet).getRow(r).getCell(c).toString();
		}
		catch(Exception e){
		   e.printStackTrace();
		}
	...
	public static int getXLRowCount(String path,String sheet)
	{
		...
		try {
			Workbook v=WorkbookFactory.create(new FileInputStream(path));
			count=v.getSheet(sheet).getLastRowNum();
		}
		catch(Exception e){
		   e.printStackTrace();
		}
